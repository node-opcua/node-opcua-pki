...
import { CA_BACKEND_VARIANTS } from "./_ca_backends";
...
// Every behaviour below is expressed through the public CertificateAuthority
// API, so it must hold whichever backend does the signing - openssl, the
// native one, or the native one driving an HSM-held key. Running the suite
// once per variant is what keeps the backends honest with each other.
for (const variant of CA_BACKEND_VARIANTS) {
    describe(`Signing Certificate with Certificate Authority (${variant.name})`, function (this: Mocha.Suite) {
        const testData = beforeTest(this);
        // every artefact this suite writes must be kept apart from the other
        // variants' - several of these files are refused rather than overwritten
        // if they already exist, which silently reuses another CA's output
        const variantDir = path.join(testData.tmpFolder, `out_${variant.name}`);

        let theCertificateAuthority: CertificateAuthority;
        let someCertificateManager: CertificateManager;

        before(async () => {
            mkdirRecursiveSync(variantDir);
            theCertificateAuthority = new CertificateAuthority({
                keySize: 2048,
                location: path.join(testData.tmpFolder, `CA_${variant.name}`),
                ...(await variant.caOptions("signing"))
            });

            someCertificateManager = new CertificateManager({
                location: path.join(testData.tmpFolder, `PI_${variant.name}`)
            });
        ...
        it("T2 - should sign a Certificate Request", async () => {
            ...
            const certificateFilename = path.join(variantDir, "sample_certificate.pem");
        ...
        async function sign(certificateRequest: Filename, startDate: Date, validity: number): Promise<string> {
            ...
            const certificate = await theCertificateAuthority.signCertificateRequest(
                certificateFilename,
                certificateRequest,
                params
            );
        ...
        it("T4 - should create various self-signed Certificates using the CA", async () => {
            ...
            const certificatePemFile = path.join(variantDir, "sample_self_signed_certificate.pem");
        ...
        it("T5 - should revoke a self-signed certificate", async () => {
            const privateKey = someCertificateManager.privateKey;
            const certificate = path.join(variantDir, "certificate_to_be_revoked1.pem");
        ...
        it("T5a2 - signCertificateRequestFromDER() returns combined chain — callers must split before toPem", async () => {
            ...
            const buggyFile = path.join(variantDir, "buggy_chain.pem");
            ...
            const fixedFile = path.join(variantDir, "fixed_chain.pem");
            let fixedPem = "";
        ...
        it("T5n - getCapabilities() advertises the same validity limits on every backend", () => {
        ...
        it("T5m - generateKeyPairAndSignPFX() should support passphrase", async () => {
            ...
            const tmpPfx = path.join(variantDir, "test_password.pfx");
        ...
        it("T6 - should revoke a certificate emitted by the CA", async () => {
            ...
            // ---- lets create a
            const pkiLocation = path.join(variantDir, "somePKI");
        ...
        it("T7 - it should automatically accept Certificate issued by a trusted issuer that is not in the CRL", async () => {
            ...
            // ---- lets create a
            const pkiLocation = path.join(variantDir, "somePKI1");
            const cm = new CertificateManager({
                location: pkiLocation
            });
            await cm.initialize();
        ...
        it("T9 - verifyCertificate() accepts what this CA signed and rejects what it did not", async () => {
            // This check runs inside signCertificateRequest, and used to do
            // nothing at all because `openssl verify` crashes on Windows. It
            // is now a pure-JS signature check, so it has to actually
            // discriminate - a foreign certificate must not pass.
            const csrFilename = await createCertificateRequest();
            const ours = path.join(variantDir, "verify_ours.pem");
            await theCertificateAuthority.signCertificateRequest(ours, csrFilename, {
                applicationUri: "urn:test:verify",
                startDate: new Date(),
                validity: 30
            });
            await theCertificateAuthority.verifyCertificate(ours);

            // a certificate from an unrelated CA must be refused
            const foreignCa = new CertificateAuthority({
                keySize: 2048,
                ...(await variant.caOptions("foreign")),
                location: path.join(variantDir, "ForeignCA"),
                subject: "/CN=Foreign CA"
            });
            await foreignCa.initialize();
            const theirs = path.join(variantDir, "verify_theirs.pem");
            await foreignCa.signCertificateRequest(theirs, csrFilename, {
                applicationUri: "urn:test:verify",
                startDate: new Date(),
                validity: 30
            });
            await theCertificateAuthority.verifyCertificate(theirs).should.be.rejectedWith(/was not signed by this/);

            // and its own CA certificate is not an end-entity certificate it issued
            await theCertificateAuthority
                .verifyCertificate(foreignCa.caCertificate)
                .should.be.rejectedWith(/was not signed by this/);
        });
    });
}

// A chain is only as portable as the backend that built it: run the whole
// root -> intermediate -> leaf hierarchy against each one.
for (const variant of CA_BACKEND_VARIANTS) {
    describe(`Intermediate CA Hierarchy (${variant.name})`, function (this: Mocha.Suite) {
        const testData = beforeTest(this);
        const variantDir = path.join(testData.tmpFolder, `hier_${variant.name}`);
        ...
        before(async () => {
            ...
            rootCA = new CertificateAuthority({
                keySize: 2048,
                ...(await variant.caOptions("RootCA")),
                location: path.join(variantDir, "RootCA"),
            ...
            intermediateCA1 = new CertificateAuthority({
                keySize: 2048,
                ...(await variant.caOptions("IntermediateCA1")),
                location: path.join(variantDir, "IntermediateCA1"),
            ...
            const signedCert1 = path.join(variantDir, "int1_signed.pem");
            ...
            intermediateCA2 = new CertificateAuthority({
                keySize: 2048,
                ...(await variant.caOptions("IntermediateCA2")),
                location: path.join(variantDir, "IntermediateCA2"),
                subject: "/CN=Intermediate CA 2/O=TestOrg"
            });
            const result2 = await intermediateCA2.initializeCSR();
            result2.status.should.eql("created");
            const signedCert2 = path.join(variantDir, "int2_signed.pem");
        ...
        it("T8c - End-entity cert signed by Intermediate CA #1 should have full chain", async () => {
            ...
            const endEntityManager = new CertificateManager({
                location: path.join(variantDir, "EndEntity1")
            });
            ...
            const certFile = path.join(variantDir, "app1_cert.pem");
        ...
        it("T8f - lifecycle: restart before install returns status 'pending'", async () => {
            ...
            const pendingCA = new CertificateAuthority({
                keySize: 2048,
                ...(await variant.caOptions("PendingCA")),
                location: path.join(variantDir, "PendingCA"),
        ...
        it("T8i - full runtime renewal flow: renewCSR → sign → install → sign cert", async () => {
            ...
            const renewedCert = path.join(variantDir, "int1_renewed.pem");
