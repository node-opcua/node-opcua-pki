const config =
    ...
    "subjectKeyIdentifier      = hash\n" +
    // -----------------------------------------------------------------------
    // OpenSSL 3.5.x strictly enforces the `always` flag in
    // `authorityKeyIdentifier` even during CSR generation (`openssl req
    // -extensions v3_ca`).  At that stage there is no issuer certificate
    // yet (we are creating the root CA), so the command fails with:
    //
    //   v3_akid.c:156: no issuer certificate
    //
    // Using plain `keyid` (without `:always`) tells OpenSSL to include
    // the authority key identifier when the issuer key is available and
    // skip it gracefully when it is not.  The extension is properly
    // populated during the self-signing step (`openssl x509 -req`).
    //
    // Tested with:
    //   - OpenSSL 3.4.1  (Windows)  — works with both values
    //   - OpenSSL 3.5.5  (Alpine)   — requires `keyid` (not `keyid:always`)
    //
    // References:
    ...
    "[ crl_ext ]\n" +
    "#issuerAltName            = issuer:copy\n" +
    // crl_ext is only used when the CA cert already exists, so
    // `keyid:always,issuer:always` is safe here.
